...
namespace MGR_Persistence.com.pe.mgr.dao.SqlString
{
    ...
    " AND USUARIO.IDDGA = ?";
        ...
        public const String MgrCompendioDetalleComboReferencia = "SELECT ID_DETALLE as codigo, NOMBRE as descripcion FROM GRTA_COMPENDIO_DETALLE\n" + 
        ...
        //Existen algunas consultas que no llaman a referencia1
        public const String MgrCompendioDetalleCombo = "SELECT ID_DETALLE as codigo, NOMBRE as descripcion FROM GRTA_COMPENDIO_DETALLE\n" +
            " WHERE ID_COMPENDIO=:id_compendio AND  \n" +
            " SYSDATE BETWEEN FECHA_INICIO_VIGENCIA AND DECODE(FECHA_FIN_VIGENCIA, '', SYSDATE)";

        //MV nuevo query para compendio y sujeto riesgo 
        public const String MgrCompendioSujetoRiesgoCombo = "SELECT ID_DETALLE codigo , NOMBRE as descripcion \n" +
             "FROM GRTA_COMPENDIO_DETALLE \n" +
             "WHERE ID_COMPENDIO=? AND SUJETO_RIESGO= TO_NUMBER(?) AND \n" +
 ...
 public const String ConsEvaluacionSelecTabla="SELECT EVALUACION.ID_MODELO||'@'||EVALUACION.ORDEN_MODELO \"Orden\", EVALUACION.NUMERO_VARIABLES \"Nro Variables\", \n" + 
  ...
  public const String TipoCategoriaCombo="SELECT ID_DETALLE, NOMBRE FROM GRTA_COMPENDIO_DETALLE \n" + 
     ...
     "ORDER BY 2";
        ...
        public const String MgrTipoOperadorCombo = "SELECT ID_DETALLE CODIGO, NOMBRE DESCRIPCION \n" +
		     "FROM GRTA_COMPENDIO_DETALLE \n" + 
		     "WHERE ID_COMPENDIO=16 AND \n" + 
		     "SYSDATE BETWEEN FECHA_INICIO_VIGENCIA AND DECODE(FECHA_FIN_VIGENCIA, '', SYSDATE)";


        public const String MgrFuenteDatosCombo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION \n" + 
        ...
        public const String MgrFuenteDatosProcesoEvaluacionCombo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION, \n" + 
					...
					"AND NUMERO_OCURRENCIAS=55 AND SYSDATE BETWEEN FECHA_INICIO_VIGENCIA AND DECODE(FECHA_FIN_VIGENCIA,'',SYSDATE)) CANTIDAD \n" + 
				     "FROM GRTA_TABLAS_SUJETO SUJETO, GRTA_COMPENDIO_DETALLE TABLA \n" + 
				     "WHERE TABLA.ID_DETALLE = SUJETO.TABLA_DATOS AND SUJETO.ORIGEN_FUENTE = 504 AND SUJETO.SUJETO_RIESGO = ?";


        public const String MgrFuenteDatosProgramaFizcalizacionCombo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION, \n" + 
					    "(SELECT COUNT(0) \n" + 
					    "FROM GRTA_VARIABLES \n" + 
					    "WHERE TIPO_VARIABLE=57 AND MODO_USO=22 AND SUJETO_RIESGO = SUJETO.SUJETO_RIESGO \n" + 
					    "AND TABLA_TRANSACCIONAL = SUJETO.TABLA_DATOS AND EXPRESION_TRANSACCIONAL NOT LIKE '%NO DEFINIDO%' \n" + 
        ...
        public const String MgrFuenteDatosrBenfordCombo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION, \n" + 
			       "(SELECT COUNT(0) \n" + 
			       "FROM GRTA_VARIABLES \n" + 
        ...
        public const String MgrFuenteDatos1821767Combo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION, \n" + 
			      "(SELECT COUNT(0) \n" + 
			      "FROM GRTA_VARIABLES \n" + 
			      "WHERE TIPO_VARIABLE = 57 AND MODO_USO = 22 AND SUJETO_RIESGO = SUJETO.SUJETO_RIESGO \n" + 
        ...
        public const String MgrFuenteDatos1719Combo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION, \n" + 
			   "(SELECT COUNT(0) \n" + 
			   "FROM GRTA_VARIABLES \n" + 
			   "WHERE SUJETO_RIESGO = SUJETO.SUJETO_RIESGO AND TABLA_TRANSACCIONAL = SUJETO.TABLA_DATOS AND \n" + 
			   "TIPO_VARIABLE = 57 AND MODO_USO = 22 AND NUMERO_OCURRENCIAS = 55 AND SYSDATE BETWEEN FECHA_INICIO_VIGENCIA AND DECODE(FECHA_FIN_VIGENCIA,'',SYSDATE) AND EXPRESION_TRANSACCIONAL NOT LIKE '%NO DEFINIDO%') CANTIDAD \n" + 
			"FROM GRTA_TABLAS_SUJETO SUJETO, GRTA_COMPENDIO_DETALLE TABLA \n" + 
			"WHERE TABLA.ID_DETALLE = SUJETO.TABLA_DATOS AND SUJETO.ORIGEN_FUENTE = 504 AND SUJETO.SUJETO_RIESGO = ?";


        public const String MgrFuenteDatosLineaDiferenteCeroCombo = "SELECT SUJETO.TABLA_DATOS CODIGO, TABLA.NOMBRE||' ('||TABLA.CODIGO_ALTERNO||')' DESCRIPCION, \n" + 
				         "(SELECT COUNT(0) \n" + 
					 "FROM GRTA_VARIABLES \n" + 
					 "WHERE SUJETO_RIESGO = SUJETO.SUJETO_RIESGO AND TABLA_TRANSACCIONAL = SUJETO.TABLA_DATOS AND \n" + 
        ...
        public const String MgrOperadorMatematicoComboFC = "SELECT OPERADOR_MATEMATICO CODIGO, SIMBOLO_USUARIO, 0 CANTIDAD \n" + 
		             "FROM GRTA_OPERADOR_MATEMATICO \n" +
			     "WHERE TIPO_OPERADOR = ?";
        ...
        public const String MgrGrupoFuncionCombo = "SELECT ID_DETALLE CODIGO, NOMBRE DESCRIPCION \n" +
		 "FROM GRTA_COMPENDIO_DETALLE \n" + 
		 "WHERE ID_COMPENDIO=328 \n" + 
		 "AND SYSDATE BETWEEN FECHA_INICIO_VIGENCIA AND DECODE(FECHA_FIN_VIGENCIA, '', SYSDATE)";


        public const String MgrFuncionCombo = "SELECT FUNCION, SIMBOLO_USUARIO \n" + 
                     "FROM GRTA_FUNCION FUNCION \n" + 
                     "WHERE FUNCION.GRUPO_FUNCION = ?";
